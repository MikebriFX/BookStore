...
namespace Bookstore.API.Controllers
{
    ...
    public class BooksController : MainController
    {
        ...
        public async Task<IActionResult> GetBooksByCategory(int categoryId)
        {
            ...
            //System.Collections.Generic.List< Book> bookes = await _bookService.GetBooksByCategory(categoryId); ;

            if (((System.Collections.Generic.List < Book >)books).Count == 0) return NotFound();
        ...
        public async Task<ActionResult<List<Book>>> SearchBookWithCategory(string searchedValue)
        {
            ...
            if (((System.Collections.Generic.List<Book>)books).Count == 0) return NotFound("None book was founded");
